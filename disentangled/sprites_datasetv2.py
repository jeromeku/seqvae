...
#         os.getenv("TEST_TMPDIR", "/tmp"), os.path.join("disentangled_vae", "data")
#     ),
#     help="Directory where the dataset is stored.",
# )

DATA_SPRITES_URL = (
    "https://github.com/jrconway3/Universal-LPC-spritesheet/archive/master.zip"
)
...
TOPS = [
    os.path.join("torso", "shirts", "longsleeve", "male", "maroon_longsleeve.png"),
    os.path.join("torso", "shirts", "longsleeve", "male", "teal_longsleeve.png"),
    os.path.join("torso", "shirts", "longsleeve", "male", "white_longsleeve.png"),
...
ACTIONS = [Action("walk", 8, 9), Action("spellcast", 0, 7), Action("slash", 12, 6)]

Direction = namedtuple("Direction", ["name", "row_offset"])
DIRECTIONS = [Direction("west", 1), Direction("south", 2), Direction("east", 3)]
...
def download_sprites():
    ...
    data_dir = "/media/jeromeku/easystore/workspace/deep_learning/tensorflow/notebooks/src/disentangled/data"
    filepath = os.path.join(data_dir, "sprites")
...
def create_sprites_dataset(
  ...
  """
    ...
    skin_table = create_lookup(SKIN_COLORS)
    hair_table = create_lookup(HAIRSTYLES)
    top_table = create_lookup(TOPS)
    pants_table = create_lookup(PANTS)
    action_table = create_lookup(action_names)

    def process_example(attrs, act_name, act_metadata, dir_row_offset):
        """Processes a dataset row."""
        # skin_name = tf.convert_to_tensor(attrs[0]).experimental_ref()
        # hair_name = tf.convert_to_tensor(attrs[1]).experimental_ref()
        # top_name = tf.convert_to_tensor(attrs[2]).experimental_ref()
        # pants_name = tf.convert_to_tensor(attrs[3]).experimental_ref()
        skin_name = attrs[0]
        hair_name = attrs[1]
        top_name = attrs[2]
        pants_name = attrs[3]
        ...
        skin_idx = skin_table.lookup(
            tf.convert_to_tensor(skin_name).experimental_ref().deref()
        )
        hair_idx = hair_table.lookup(tf.convert_to_tensor(hair_name, dtype=tf.string))
        top_idx = top_table.lookup(tf.convert_to_tensor(top_name, dtype=tf.string))
        pants_idx = pants_table.lookup(
            tf.convert_to_tensor(pants_name, dtype=tf.string)
        )
        act_idx = action_table.lookup(tf.convert_to_tensor(act_name, tf.string))

        # print(f"DEBUG SKIN_IDX: {skin_idx}")
        return (
            seq,
            skin_idx,
            hair_idx,
  ...
  """
    ...
    """
        ...
        if num_train_actual != num_train:
            raise ValueError(
                "Unexpected number of training examples: {}.".format(num_train_actual)
            )
        if num_test_actual != num_test:
            raise ValueError(
                "Unexpected number of testing examples: {}.".format(num_test_actual)
            )
